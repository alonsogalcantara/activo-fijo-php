...
<div class="flex justify-between items-center mb-6 flex-wrap gap-4">
    <div>
        ...
        <p class="text-gray-500 text-sm">Gestión de flota, equipo de cómputo y mobiliario.</p>
    </div>
    <a href="/assets/create"
        class="bg-blue-600 text-white px-5 py-2.5 rounded-xl hover:bg-blue-700 transition shadow-lg hover:shadow-xl flex items-center font-bold">
        <i class="fas fa-plus mr-2"></i> Registrar Activo
    </a>
</div>

<!-- BARRA DE BÚSQUEDA Y FILTROS MEJORADA -->
<div class="bg-white p-4 rounded-xl shadow-sm mb-6 border border-gray-100 flex flex-col md:flex-row gap-4">
    <div class="relative flex-1">
        <i class="fas fa-search absolute left-4 top-3.5 text-gray-400"></i>
        <input type="text" id="assetSearchInput" placeholder="Buscar por nombre, serie, usuario, placa..."
            class="w-full pl-12 pr-4 py-3 border border-gray-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-blue-500 bg-gray-50 focus:bg-white transition"
            onkeyup="filterAssets()">
    </div>

    <div class="w-full md:w-40">
        <select id="filterCategory" onchange="filterAssets()"
            class="w-full p-3 border border-gray-200 rounded-xl bg-gray-50 focus:bg-white focus:outline-none focus:ring-2 focus:ring-blue-500 cursor-pointer text-gray-700 text-sm">
...
<div class="bg-white rounded-xl shadow overflow-hidden w-full border border-gray-100">
    <div class="overflow-x-auto">
        <table class="w-full text-left border-collapse" id="assetsTable">
            <thead
                class="bg-slate-50 text-slate-500 uppercase text-xs font-bold tracking-wider border-b border-gray-200">
                <tr>
                    <th class="p-4 cursor-pointer hover:bg-slate-100 transition" onclick="sortTable('assetsTable', 0, 'date')">Fecha Registro</th>
                    <th class="p-4 w-20 text-center">Foto</th>
                    <th class="p-4 cursor-pointer hover:bg-slate-100 transition" onclick="sortTable('assetsTable', 2)">Descripción</th>
                    <th class="p-4 cursor-pointer hover:bg-slate-100 transition" onclick="sortTable('assetsTable', 3)">Adquisición</th>
                    <th class="p-4 cursor-pointer hover:bg-slate-100 transition" onclick="sortTable('assetsTable', 4)">Identificador</th>
                    <th class="p-4 cursor-pointer hover:bg-slate-100 transition" onclick="sortTable('assetsTable', 5)">Ubicación / Asignación</th>
                    <th class="p-4 text-center cursor-pointer hover:bg-slate-100 transition" onclick="sortTable('assetsTable', 6)">Estado</th>
                    <th class="p-4 text-center">Acciones</th>
                </tr>
            </thead>
            <tbody class="divide-y divide-gray-50 bg-white" id="assetsTableBody">
                <?php if (!empty($assets)): ?>
                <?php foreach ($assets as $a): ?>
                <tr class="hover:bg-blue-50/50 transition group asset-row" data-category="<?= htmlspecialchars($a['category']) ?>"
                    ...
                    <td class="p-4 text-center">
                        <div
                            class="flex justify-center items-center gap-2 opacity-80 group-hover:opacity-100 transition">
                            <!-- View Detail -->
                            <a href="/assets/detail/<?= $a['id'] ?>"
                                class="w-8 h-8 rounded-full bg-white border border-gray-200 text-blue-500 hover:bg-blue-50 hover:border-blue-300 flex items-center justify-center transition shadow-sm"
                                title="Ver Detalle"><i class="fas fa-eye"></i></a>
                                
                            <a href="/assets/edit/<?= $a['id'] ?>"
                                class="w-8 h-8 rounded-full bg-white border border-gray-200 text-yellow-600 hover:bg-yellow-50 hover:border-yellow-300 flex items-center justify-center transition shadow-sm"
                                title="Editar"><i class="fas fa-pen"></i></a>
                            
                            <a href="/assets/delete/<?= $a['id'] ?>" onclick="return confirm('¿Estás seguro?')"
                                class="w-8 h-8 rounded-full bg-white border border-gray-200 text-red-500 hover:bg-red-50 hover:border-red-300 flex items-center justify-center transition shadow-sm"
